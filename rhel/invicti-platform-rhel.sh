#!/usr/bin/env bash
#===============================================================================
#  invicti-platform-rhel.sh — Invicti Platform on-premises lifecycle manager
#                              for Red Hat Enterprise Linux and rebuilds
#===============================================================================
#  Installs, upgrades, backs up, restores and removes the Invicti Platform
#  on-premises edition on RHEL 9, Rocky Linux 9, AlmaLinux 9, CentOS Stream 9
#  and Oracle Linux 9, onto a single-node k3s or RKE2 cluster, or onto any
#  existing Kubernetes cluster. Follows the official Helm documentation:
#      https://docs.invicti.com/ip/category/invicti-platform-on-premises
#
#  COMMANDS
#    install        Preflight, install k3s/RKE2 + Helm 3 if needed, deploy the chart
...
#    version        Print script, chart, Helm, cluster and Kubernetes versions
#
#  QUICK START
...
#
#  KUBERNETES FLAVOUR
#    --k8s k3s     single-node k3s, SELinux policy installed automatically (default)
#    --k8s rke2    RKE2, the CIS-hardened distribution more usual in RHEL estates
#
#  NON-INTERACTIVE
#    INVICTI_EMAIL=... INVICTI_LICENSE_KEY=... PLATFORM_HOST=... \
#      ./invicti-platform-rhel.sh install --yes
#
...
#    * Red Hat's guided LVM install leaves part of the disk unallocated, which
...
#
#  WHY THIS EXISTS (the Red Hat specifics)
#    * SELinux is Enforcing out of the box. Without the k3s-selinux policy the
#      containers fail on their volume mounts with permission denied, and it
#      reads like a storage fault rather than a policy one. The k3s installer
#      pulls that policy itself; this script makes sure container-selinux is in
#      place first and refuses to quietly drop the host to permissive.
#    * firewalld is running out of the box and drops the pod and service CIDRs,
#      so DNS inside the cluster fails and every pod stalls in CrashLoopBackOff.
#      Upstream's advice is to disable it; this script instead trusts 10.42/16
#      and 10.43/16 and opens the API port, which keeps the host firewall on.
#    * RKE2's Canal CNI needs NetworkManager told to leave the CNI interfaces
#      alone, otherwise NM reclaims them and the cluster network dies at the
#      next reload. The drop-in goes in before RKE2 starts, never after.
#    * On subscription-managed RHEL with no active entitlement there are no
#      repositories at all, so the first dnf call fails with a wall of noise.
#      This script checks entitlement first and says so in one line.
#===============================================================================

set -Eeuo pipefail

SCRIPT_VERSION="1.0.0-rhel"
...
K8S_DIST="${K8S_DIST:-k3s}"                 # k3s | rke2
SELINUX_MODE="${SELINUX_MODE:-enforcing}"   # enforcing | permissive
FIREWALLD_MODE="${FIREWALLD_MODE:-configure}"  # configure | disable | skip
EXPAND_DISK="${EXPAND_DISK:-ask}"           # ask | yes | no
AUTO_FALLBACK="${AUTO_FALLBACK:-true}"
RESOURCE_PROFILE="${RESOURCE_PROFILE:-}"
SKIP_UPGRADE="${SKIP_UPGRADE:-true}"        # dnf upgrade off by default
...
PURGE_K3S="${PURGE_K3S:-false}"             # remove k3s / rke2 entirely
...
# Red Hat family facts, resolved once by detect_os() and used throughout.
OS_ID=""            # rhel | rocky | almalinux | centos | ol
OS_NAME=""
OS_MAJOR=""
IS_RHEL_SUBSCRIBED=""   # true only on genuine RHEL carrying subscription-manager
...
# k3s writes /etc/rancher/k3s/k3s.yaml; RKE2 writes /etc/rancher/rke2/rke2.yaml
# and keeps its own kubectl out of $PATH. Resolve both here so every later
# function can stay flavour-agnostic.
cluster_kubeconfig_src() {
  if [[ -f /etc/rancher/rke2/rke2.yaml ]] && [[ "$K8S_DIST" == "rke2" || ! -f /etc/rancher/k3s/k3s.yaml ]]; then
    printf '/etc/rancher/rke2/rke2.yaml'
  else
    printf '/etc/rancher/k3s/k3s.yaml'
  fi
}

# RKE2 does not put kubectl on the PATH. Add its bin directory when present so
# the operator's shell and this script both find it.
ensure_kubectl_path() {
  [[ -d /var/lib/rancher/rke2/bin ]] || return 0
  [[ ":$PATH:" == *":/var/lib/rancher/rke2/bin:"* ]] && return 0
...
setup_kubeconfig() {
  ...
  # cluster); fall back to the one the local distribution wrote.
  ensure_kubectl_path
  ...
  local src; src="$(cluster_kubeconfig_src)"
  if [[ -r "$src" ]]; then
...
install_kubeconfig_for_user() {
  ...
  $SUDO cp "$(cluster_kubeconfig_src)" "$TARGET_HOME/.kube/config"
...
#-------------------------------------------------------------- OS detection
# Resolve the distribution once. Everything downstream keys off OS_ID and
# OS_MAJOR rather than re-parsing /etc/os-release, so there is one place to
# change when a new rebuild appears.
detect_os() {
  [[ -n "$OS_ID" ]] && return 0
  [[ -r /etc/os-release ]] || die "/etc/os-release is missing; cannot identify this distribution."
  # Read it in a subshell so its variables never leak into the script's scope.
  OS_ID="$(   . /etc/os-release; printf '%s' "${ID:-}" )"
  OS_NAME="$( . /etc/os-release; printf '%s' "${PRETTY_NAME:-unknown}" )"
  OS_MAJOR="$(. /etc/os-release; printf '%s' "${VERSION_ID:-}" | cut -d. -f1 )"
  local like; like="$( . /etc/os-release; printf '%s' "${ID_LIKE:-}" )"

  info "OS: ${OS_NAME}   kernel $(uname -r)"

  case "$OS_ID" in
    rhel|rocky|almalinux|centos|ol)  : ;;
    *)
      if [[ "$like" == *rhel* || "$like" == *fedora* ]]; then
        warn "'${OS_ID}' is not a distribution this script has been tested on, but it is RHEL-like; continuing."
      else
        bad "'${OS_NAME}' is not a Red Hat family distribution."
        hint "This build targets RHEL, Rocky, AlmaLinux, CentOS Stream and Oracle Linux."
        hint "For Debian or Ubuntu use the sister script in this repository:"
        hint "    debian/invicti-platform.sh"
        die "Wrong distribution family for this script."
      fi ;;
  esac

  case "$OS_MAJOR" in
    9)  ok "Red Hat family, version ${OS_MAJOR}: supported." ;;
    10) warn "Version 10 is newer than this script has been tested against; continuing." ;;
    8)  warn "Version 8 reaches the end of full support sooner and defaults to cgroup v1."
        hint "The chart works, but resource accounting is less accurate than on 9." ;;
    *)  warn "Version '${OS_MAJOR:-unknown}' is untested; 9 is the target." ;;
  esac

  command -v dnf >/dev/null || die "dnf not found. This script needs a dnf-based Red Hat system."
}

# Genuine RHEL without an entitlement has no repositories at all, so the first
# dnf transaction fails with a wall of 'Failed to download metadata'. Catch it
# here and say the one thing that actually needs doing. Rebuilds such as Rocky
# and AlmaLinux carry no subscription-manager and are skipped.
check_subscription() {
  IS_RHEL_SUBSCRIBED="false"
  [[ "$OS_ID" == "rhel" ]] || return 0
  command -v subscription-manager >/dev/null || return 0
  IS_RHEL_SUBSCRIBED="true"
  local status
  status="$($SUDO subscription-manager status 2>&1 || true)"
  if grep -qiE 'Overall Status: *(Current|Registered|Disabled)' <<<"$status"; then
    ok "RHEL subscription is active."
  elif grep -qiE 'Overall Status: *Unknown|not registered|No such' <<<"$status"; then
    bad "This RHEL host is not registered, so dnf has no repositories to install from."
    hint "Register it first, then re-run:"
    hint "    sudo subscription-manager register --username <user> --auto-attach"
    hint "Or, if you use Satellite or a local mirror, point dnf at that and re-run."
  else
    warn "Could not read the subscription status; package installation may fail."
  fi
  # Confirm there is at least one enabled repository, which is what actually matters.
  if ! $SUDO dnf repolist --quiet 2>/dev/null | grep -qE '^[a-z]'; then
    bad "No enabled dnf repositories. Package installation will fail."
  fi
}

# SELinux is the single biggest difference from Debian. Report it plainly at
# preflight so the operator is never surprised later, and never silently weaken it.
report_selinux() {
  if ! command -v getenforce >/dev/null; then
    warn "SELinux tools are not installed; assuming SELinux is disabled."
    return 0
  fi
  local mode; mode="$(getenforce 2>/dev/null || echo Unknown)"
  case "$mode" in
    Enforcing)
      ok "SELinux: Enforcing (the k3s/RKE2 policy will be installed to match)." ;;
    Permissive)
      warn "SELinux: Permissive. The cluster will run, but the host is not enforcing policy." ;;
    Disabled)
      warn "SELinux: Disabled. Fine for a lab; most Red Hat estates require it Enforcing." ;;
    *)
      warn "SELinux: ${mode}." ;;
  esac
}
...
preflight() {
  ...
  # A minimal Red Hat install ships neither tar nor jq, and install_packages
  # runs after this check. Treating them as hard failures deadlocks the script
  # against the very step that fixes them, so anything in BASE_PACKAGES that dnf
  # can supply is reported as pending rather than failed. Only report a genuine
  # failure when the package step cannot run at all.
  local t missing_base=()
  for t in curl tar jq python3; do
  ...
  if (( ${#missing_base[@]} > 0 )); then
    if [[ "$COMMAND" == "check" ]]; then
      warn "Not installed yet: ${missing_base[*]}"
      hint "The install command installs these from BaseOS/AppStream before it needs them."
    elif command -v dnf >/dev/null; then
      info "Will install from BaseOS/AppStream: ${missing_base[*]}"
    else
      bad "Missing and no dnf to install them: ${missing_base[*]}"
    fi
  fi
  ...
  if command -v kubectl >/dev/null; then
    ...
    info "kubectl not installed; ${K8S_DIST} provides it once the cluster is up."
  fi
...
# dnf takes a transaction lock rather than a lock file pair, and on a fresh
# Red Hat box dnf-makecache.timer frequently holds it for the first few minutes
# after boot. Wait for it rather than failing the install on a race.
wait_for_dnf() {
  local waited=0
  while $SUDO fuser /var/lib/rpm/.rpm.lock >/dev/null 2>&1 \
     || $SUDO fuser /var/cache/dnf/metadata_lock.pid >/dev/null 2>&1 \
     || pgrep -x dnf >/dev/null 2>&1 \
     || pgrep -x packagekitd >/dev/null 2>&1; do
    (( waited == 0 )) && info "Waiting for another dnf transaction to finish ..."
    sleep 5; waited=$((waited+5))
    (( waited > 600 )) && die "dnf has been locked for 10 minutes. Check: ps aux | grep -E 'dnf|packagekit'"
  done
}

# Base packages. Every one of these is in BaseOS or AppStream on RHEL 9 and its
# rebuilds, so no EPEL is required. Verified on Rocky 9.8, where jq ships as
# jq-1.6-19.el9. Debian's open-iscsi is iscsi-initiator-utils here, and python3
# is listed explicitly because the backup, restore and CRD-cleanup paths parse
# JSON with it and a minimal install does not always carry it.
BASE_PACKAGES=(curl tar ca-certificates iscsi-initiator-utils lvm2 jq python3)

install_packages() {
  command -v dnf >/dev/null || { warn "Not a dnf system; skipping the package step."; return 0; }
  log "Base packages"

  # Skip entirely when everything is already present, so a re-install on a
  ...
  # curl is 'curl-minimal' on a minimal Red Hat install, which satisfies the
  # dependency perfectly well. Do not drag in the full curl and force a swap.
  if [[ " ${missing[*]} " == *" curl "* ]] && command -v curl >/dev/null; then
    missing=("${missing[@]/curl}")
  fi
  # Collapse the empty slot the substitution above can leave behind.
  local cleaned=(); for pkg in "${missing[@]}"; do [[ -n "$pkg" ]] && cleaned+=("$pkg"); done
  missing=("${cleaned[@]}")
  ...
  if [[ "$SKIP_UPGRADE" != "true" ]]; then
    ...
    wait_for_dnf; run $SUDO dnf -y -q upgrade
  fi

  if (( ${#missing[@]} > 0 )); then
    wait_for_dnf
    run $SUDO dnf -y -q install "${missing[@]}" \
      || die "dnf could not install: ${missing[*]}. Check 'dnf repolist' and your network."
  fi
  ok "Base packages present."

  # A kernel update needs a reboot before the new modules are in use. Say so
  # rather than letting the operator discover it after a confusing failure.
  if command -v needs-restarting >/dev/null && ! $SUDO needs-restarting -r >/dev/null 2>&1; then
    warn "A reboot is pending from a kernel or core library update."
    hint "Continue now if you like; reboot when convenient."
  fi
  return 0
}

#-------------------------------------------------------------- SELinux
# k3s and RKE2 both ship an SELinux policy module. Their installers fetch it
# themselves when the host has network access; all this needs to do is make sure
# container-selinux is present first, because the policy RPM depends on it, and
# to be explicit when the operator has asked to relax enforcement.
configure_selinux() {
  command -v getenforce >/dev/null || return 0
  local mode; mode="$(getenforce 2>/dev/null || echo Disabled)"

  if [[ "$SELINUX_MODE" == "permissive" && "$mode" == "Enforcing" ]]; then
    warn "Setting SELinux to Permissive for this boot because --selinux permissive was given."
    hint "This is a runtime change only; /etc/selinux/config is left alone, so a"
    hint "reboot returns the host to Enforcing. Prefer fixing policy over this."
    need_root
  ...
  if rpm -q container-selinux >/dev/null 2>&1; then
    ok "container-selinux present."
    return 0
  fi
  log "Installing container-selinux"
  hint "The k3s and RKE2 policy modules depend on it. Without them the containers"
  hint "fail on their volume mounts with permission denied, which reads like a"
  hint "storage fault rather than an SELinux one."
  need_root
  wait_for_dnf
  run $SUDO dnf -y -q install container-selinux \
    || die "Could not install container-selinux, which the cluster's SELinux policy needs."
  ok "container-selinux installed."
}

#-------------------------------------------------------------- firewalld
# firewalld is active on a default Red Hat install and silently drops the pod
# and service networks, so cluster DNS fails and every pod ends in
# CrashLoopBackOff. Upstream's advice is to disable firewalld outright; this
# trusts the two cluster CIDRs instead, which keeps the host firewall doing its
# job for everything else.
configure_firewalld() {
  [[ "$FIREWALLD_MODE" == "skip" ]] && { info "Leaving firewalld untouched (--firewalld skip)."; return 0; }
  command -v firewall-cmd >/dev/null || return 0
  $SUDO systemctl is-active --quiet firewalld 2>/dev/null || { ok "firewalld is not running."; return 0; }

  need_root
  if [[ "$FIREWALLD_MODE" == "disable" ]]; then
    log "Disabling firewalld"
    warn "Disabling the host firewall entirely, as requested."
    run $SUDO systemctl disable --now firewalld
    ok "firewalld disabled."
    return 0
  fi

  log "Configuring firewalld for ${K8S_DIST}"
  # Pod CIDR and service CIDR are the same defaults for k3s and RKE2.
  run $SUDO firewall-cmd --permanent --zone=trusted --add-source=10.42.0.0/16
  run $SUDO firewall-cmd --permanent --zone=trusted --add-source=10.43.0.0/16
  run $SUDO firewall-cmd --permanent --add-port=6443/tcp      # kube-apiserver
  run $SUDO firewall-cmd --permanent --add-port=443/tcp       # platform ingress
  run $SUDO firewall-cmd --permanent --add-port=80/tcp        # platform redirect
  if [[ "$K8S_DIST" == "rke2" ]]; then
    run $SUDO firewall-cmd --permanent --add-port=9345/tcp                 # RKE2 supervisor
    run $SUDO firewall-cmd --permanent --add-port=30000-32767/tcp          # NodePort range
  fi
  [[ "$SERVICE_TYPE" == "NodePort" ]] && run $SUDO firewall-cmd --permanent --add-port="${NODE_PORT}/tcp"
  run $SUDO firewall-cmd --reload
  ok "firewalld now trusts the cluster networks; the host firewall stays on."
}

#-------------------------------------------------------------- NetworkManager
# RKE2's Canal CNI creates flannel and calico interfaces. NetworkManager will
# claim them and tear the cluster network down at its next reload unless it is
# told to leave them alone. The drop-in must land before RKE2 first starts;
# applying it afterwards needs a full reboot to take effect.
NM_DROPIN=/etc/NetworkManager/conf.d/rke2-canal.conf

configure_networkmanager() {
  [[ "$K8S_DIST" == "rke2" ]] || return 0
  command -v nmcli >/dev/null || return 0
  $SUDO systemctl is-active --quiet NetworkManager 2>/dev/null || return 0

  if [[ -f "$NM_DROPIN" ]]; then
    ok "NetworkManager already told to ignore the CNI interfaces."
  else
    log "Telling NetworkManager to ignore the CNI interfaces"
    need_root
    $SUDO tee "$NM_DROPIN" >/dev/null <<'NMEOF'
[keyfile]
...
NMEOF
  ...
  # Present on cloud images and on RHEL 8.4-era NetworkManager. It rewrites
  # routes from cloud metadata and breaks CNI routing after a reboot.
  local u
  for u in nm-cloud-setup.service nm-cloud-setup.timer; do
    if $SUDO systemctl is-enabled "$u" >/dev/null 2>&1; then
      warn "${u} is enabled and is known to break CNI routing."
      need_root
      run $SUDO systemctl disable --now "$u"
      warn "Disabled ${u}. A reboot is recommended before relying on this node."
    fi
  done
}
...
EOF
  ...
  # --selinux keeps the cluster running under an Enforcing host policy. The
  # installer pulls k3s-selinux from rpm.rancher.io itself; container-selinux is
  # already in place from configure_selinux.
  local k3s_exec="server --disable=traefik"
  if [[ "$(getenforce 2>/dev/null || echo Disabled)" == "Enforcing" ]]; then
    k3s_exec+=" --selinux"
    info "Enabling k3s SELinux support to match the Enforcing host policy."
  fi
  run bash -c "curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC=\"${k3s_exec}\" sh -"
  hash -r
  install_kubeconfig_for_user
...
#-------------------------------------------------------------- RKE2
# RKE2 differs from k3s in three ways that matter to this chart:
#   1. it bundles ingress-nginx, which claims host ports 80 and 443 exactly as
#      k3s's Traefik does, so the chart's own nginx-service can never bind them;
#   2. it does not enable ServiceLB, so a LoadBalancer service stays <pending>
#      for ever on a single node;
#   3. it ships no storage provisioner at all, and the chart requires a default
#      StorageClass that can provision dynamically.
# All three are handled here, before the server is first started, because
# changing them afterwards means restarting the cluster.
...
write_rke2_config() {
  ...
  if [[ -f "$RKE2_CONFIG" ]] && grep -q 'invicti-platform-rhel' "$RKE2_CONFIG" 2>/dev/null; then
    ok "RKE2 config already written by this script."
    return 0
  fi
  [[ -f "$RKE2_CONFIG" ]] && {
    warn "An existing ${RKE2_CONFIG} is present; backing it up before writing ours."
    run $SUDO cp -a "$RKE2_CONFIG" "${RKE2_CONFIG}.bak.$(date +%s)"
  }
  local selinux_line="selinux: false"
  [[ "$(getenforce 2>/dev/null || echo Disabled)" == "Enforcing" ]] && selinux_line="selinux: true"
  $SUDO tee "$RKE2_CONFIG" >/dev/null <<RKEEOF
# Written by invicti-platform-rhel. Backed up any file found here first.
# ingress-nginx is disabled because the Invicti chart publishes its own
# nginx-service on 80/443; two ingress controllers cannot share those ports.
disable:
  - rke2-ingress-nginx
# ServiceLB gives a single node a working LoadBalancer implementation, which the
# chart's nginx-service needs in order to receive an external address.
enable-servicelb: true
${selinux_line}
RKEEOF
  ok "Wrote ${RKE2_CONFIG}."
}

# RKE2 has no built-in dynamic provisioner. The chart needs a default
# StorageClass supporting ReadWriteOnce, so install Rancher's local-path
# provisioner and mark it default. This is the same component k3s bundles.
# The local-path provisioner writes its volumes to a host directory, and its
# helper pod does the mkdir from inside a container. On a Red Hat system that
# directory inherits usr_t, which container_t cannot write to, so every helper
# pod dies with "mkdir: Permission denied", no PVC ever binds, and the whole
# platform sits in CrashLoopBackOff behind a database that never got a volume.
# The failure names storage, never SELinux, which is what makes it expensive.
#
# k3s never hits this: its bundled provisioner uses /var/lib/rancher/k3s/storage,
# which the k3s-selinux policy already labels and relabels on install.
#
# The subtle part: container-selinux already ships a file-context rule mapping
# /opt/local-path-provisioner to container_file_t. The rule alone is not enough,
# because the directory is created at runtime by the provisioner and inherits
# usr_t from /opt rather than picking up the policy default. Nothing ever runs
# restorecon on it, so the label never gets applied. Creating the directory and
# relabelling it here, before the first volume is requested, is the actual fix.
# The semanage rule below is belt and braces for policy versions that lack it,
# and the uninstall path removes only the custom rule this script added.
LOCAL_PATH_DIR="${LOCAL_PATH_DIR:-/opt/local-path-provisioner}"

prepare_local_path_selinux() {
  need_root
  $SUDO mkdir -p "$LOCAL_PATH_DIR"
  command -v getenforce >/dev/null || return 0
  [[ "$(getenforce 2>/dev/null)" == "Enforcing" ]] || return 0

  log "Labelling ${LOCAL_PATH_DIR} for container access"
  if ! command -v semanage >/dev/null; then
    info "Installing policycoreutils-python-utils, which provides semanage."
    wait_for_dnf
    $SUDO dnf -y -q install policycoreutils-python-utils >/dev/null 2>&1 || true
  fi
  if command -v semanage >/dev/null; then
    # -a fails if the rule already exists, in which case modify it instead.
    $SUDO semanage fcontext -a -t container_file_t "${LOCAL_PATH_DIR}(/.*)?" 2>/dev/null \
      || $SUDO semanage fcontext -m -t container_file_t "${LOCAL_PATH_DIR}(/.*)?" 2>/dev/null \
      || warn "Could not register an fcontext rule for ${LOCAL_PATH_DIR}."
    run $SUDO restorecon -R "$LOCAL_PATH_DIR"
  else
    warn "semanage is unavailable; using chcon, which a relabel will undo."
    run $SUDO chcon -Rt container_file_t "$LOCAL_PATH_DIR"
  fi
  # stat -c %C reports the context directly. Parsing `ls -ldZ` by field number
  # is fragile: the context is field 5, not 4, and an earlier version of this
  # check printed the group name and reported a false failure.
  local ctx; ctx="$($SUDO stat -c %C "$LOCAL_PATH_DIR" 2>/dev/null || echo unknown)"
  if [[ "$ctx" == *container_file_t* ]]; then
    ok "${LOCAL_PATH_DIR} is labelled container_file_t."
  else
    bad "${LOCAL_PATH_DIR} is still labelled '${ctx:-unknown}'."
    hint "PVCs will stay Pending with 'mkdir: Permission denied' in the helper pod."
  fi
}

ensure_default_storageclass() {
  kubectl get storageclass >/dev/null 2>&1 || return 0
  if kubectl get storageclass -o jsonpath='{range .items[*]}{.metadata.annotations.storageclass\.kubernetes\.io/is-default-class}{"\n"}{end}' 2>/dev/null | grep -q true; then
    ok "A default StorageClass is already present."
    return 0
  fi
  # Label the host directory before the provisioner is asked for a volume,
  # otherwise the first several PVCs fail and sit in the provisioner's backoff.
  prepare_local_path_selinux
  log "Installing the local-path storage provisioner"
  hint "RKE2 ships no provisioner. The chart needs a default StorageClass that can"
  hint "provision ReadWriteOnce volumes dynamically, so install the same one k3s bundles."
  ...
  for i in $(seq 1 30); do
    ...
    (( i == 30 )) && die "local-path StorageClass did not appear."
    sleep 2
  done
...
install_rke2() {
  if command -v rke2 >/dev/null && $SUDO systemctl is-active --quiet rke2-server 2>/dev/null; then
    ok "RKE2 already installed and running."
    ensure_kubectl_path
    return 0
  fi
  if [[ "$INSTALL_K3S" == "no" ]]; then
    die "No cluster available and --no-k3s was given. Point KUBECONFIG at a cluster, or allow RKE2."
  fi
  if [[ "$INSTALL_K3S" == "auto" && "$ASSUME_YES" != "true" ]]; then
    confirm "No Kubernetes cluster found. Install single-node RKE2 on this host?" \
      || die "Aborted. Provide a cluster via KUBECONFIG, or re-run with --install-k3s."
  fi
  need_root
  configure_networkmanager
  ...
  if [[ -n "${PROXY_HTTPS:-${https_proxy:-${HTTPS_PROXY:-}}}" ]]; then
    info "Proxy detected — writing /etc/default/rke2-server"
...
EOF
  ...
  write_rke2_config
  run bash -c 'curl -sfL https://get.rke2.io | INSTALL_RKE2_TYPE=server sh -'
  run $SUDO systemctl enable --now rke2-server.service
  ensure_kubectl_path
  install_kubeconfig_for_user

  info "Waiting for the node to become Ready (RKE2 takes longer than k3s) ..."
  local i
  for i in $(seq 1 120); do
    kubectl get nodes 2>/dev/null | grep -qw Ready && break
    (( i == 120 )) && die "Node not Ready after 10 minutes. Inspect: sudo journalctl -u rke2-server -e"
    sleep 5
  done
  ok "RKE2 ready: $(kubectl get nodes --no-headers | awk '{print $1, $5}')"
  ensure_default_storageclass
}

# k3s installs from get.k3s.io and drops its helper scripts in /usr/local/bin.
# RKE2 installs from an RPM on Red Hat, and the RPM owns /usr/bin/rke2-*.sh
# instead. Looking in only one of those silently skips the uninstall and leaves
# a running cluster behind, so check every location either method uses.
find_cluster_script() {
  local name="$1" d
  for d in /usr/local/bin /usr/bin /opt/rke2/bin /opt/bin; do
    [[ -x "${d}/${name}" ]] && { printf '%s' "${d}/${name}"; return 0; }
  done
  return 1
}

# Both k3s and RKE2 leave netfilter state and virtual interfaces behind when
# their uninstallers run, which is why upstream tells you to reboot afterwards.
# Installing a cluster on top of that residue produces a failure that points
# nowhere near the cause: pods cannot reach the service network at all and
# report "no route to host", while the host itself reaches the very same
# address without trouble. kube-proxy's rules are present and correct, the
# firewall is configured correctly, and it still does not work.
#
# Worse, it is not self-healing. The chart's certificate and secret generator
# Jobs exhaust their backoff limit while the network is broken, and a Kubernetes
# Job that has Failed is never retried, so the platform stays down even after
# connectivity comes back. Recovering means uninstall --purge and a reinstall.
#
# Only checked when no cluster is currently running, since a healthy cluster
# legitimately owns these interfaces.
check_stale_cni() {
  local stale=()
  if command -v nft >/dev/null && $SUDO nft list tables 2>/dev/null | grep -qi 'calico'; then
    stale+=("Calico nftables tables")
  fi
  if ip -br link 2>/dev/null | grep -qE '^(cali[0-9a-f]|flannel\.|cni0|vxlan\.calico)'; then
    stale+=("CNI interfaces ($(ip -br link | grep -oE '^(cali[0-9a-f]+|flannel\.[0-9]+|cni0|vxlan\.calico)' | tr '\n' ' '))")
  fi
  (( ${#stale[@]} == 0 )) && return 0

  warn "Leftover network state from a previous Kubernetes install is present:"
  local x; for x in "${stale[@]}"; do hint "  - ${x}"; done
  hint "Neither the k3s nor the RKE2 uninstaller clears all of this, and installing"
  hint "over it breaks pod networking in a way that looks like a firewall or storage"
  hint "fault rather than leftover state. Reboot this host, then run install again."
  if [[ "$FORCE" != "true" ]]; then
    if [[ "$ASSUME_YES" == "true" ]]; then
      die "Refusing to install over leftover cluster network state. Reboot first, or pass --force."
    fi
    confirm "Install anyway? (a reboot first is strongly recommended)" \
      || die "Aborted. Reboot, then re-run: $SCRIPT_NAME install"
  fi
}

# One entry point so callers never care which flavour is in play.
install_cluster() {
  check_stale_cni
  case "$K8S_DIST" in
    k3s)  install_k3s  ;;
    rke2) install_rke2 ;;
    *)    die "Unknown --k8s value '${K8S_DIST}'. Use k3s or rke2." ;;
  esac
}
...
# RKE2's equivalent (ingress-nginx) is disabled in config.yaml before first
# start instead, so this is a no-op there.
disable_traefik() {
  [[ "$K8S_DIST" == "k3s" ]] || return 0
...
install_helm() {
  ...
  hash -r
  [[ "$(helm_major)" == "3" ]] || die "Still resolving to Helm $(helm version --short). Ensure /usr/local/bin comes first in PATH."
  ok "Helm $(helm version --short) installed."
}

# Reuse the credentials already on disk. Without this, `upgrade` and
# `reconfigure` demand --email and --license again even though values.yaml has
# held both since the install, which makes routine operations need the license
# key on the command line and encourages people to paste it into shell history.
reuse_saved_credentials() {
  [[ -r "$VALUES_FILE" ]] || return 0
  if [[ -z "$INVICTI_EMAIL" ]]; then
    INVICTI_EMAIL="$(awk -F': *' '/^[[:space:]]*email_address:/ {print $2; exit}' "$VALUES_FILE" 2>/dev/null | tr -d '"'"'"'[:space:]')"
    [[ -n "$INVICTI_EMAIL" ]] && info "Reusing the account email from ${VALUES_FILE}."
  fi
  if [[ -z "$INVICTI_LICENSE_KEY" ]]; then
    INVICTI_LICENSE_KEY="$(awk -F': *' '/^[[:space:]]*license_key:/ {print $2; exit}' "$VALUES_FILE" 2>/dev/null | tr -d '"'"'"'[:space:]')"
    [[ -n "$INVICTI_LICENSE_KEY" ]] && info "Reusing the license key from ${VALUES_FILE}."
  fi
}
...
cmd_install() {
  ...
  configure_selinux
  configure_firewalld
  if ! setup_kubeconfig; then install_cluster; fi
  ensure_default_storageclass
  install_helm
  require_cluster
  disable_traefik
...
#    --purge-all       the above plus the local cluster (k3s or RKE2)
#===============================================================================
...
' 2>/dev/null || true)"
  ...
  if [[ "$PURGE_K3S" == "true" ]]; then
    log "Removing the Kubernetes distribution"
    confirm "Really uninstall the local Kubernetes cluster from this host?" || die "Aborted."
    need_root
    local removed=false
    local unins killer

    if unins="$(find_cluster_script k3s-uninstall.sh)"; then
      run $SUDO "$unins"
      # k3s's own uninstaller leaves /etc/rancher/node behind, which makes a
      # later run look like a cluster is still configured.
      $SUDO rm -rf /etc/rancher/node 2>/dev/null || true
      ok "k3s removed."; removed=true
    fi

    if unins="$(find_cluster_script rke2-uninstall.sh)"; then
      # Stop everything first. rke2-killall.sh tears down the containers and
      # network namespaces; without it the uninstaller can leave interfaces and
      # mounts behind that only a reboot clears.
      if killer="$(find_cluster_script rke2-killall.sh)"; then
        run $SUDO "$killer" || true
      fi
      $SUDO systemctl stop rke2-server 2>/dev/null || true
      run $SUDO "$unins"
      $SUDO rm -f "$RKE2_CONFIG" "$NM_DROPIN" /etc/default/rke2-server 2>/dev/null || true
      $SUDO rm -rf /etc/rancher/rke2 2>/dev/null || true
      $SUDO systemctl reload NetworkManager 2>/dev/null || true
      ok "RKE2 removed, along with its config and the NetworkManager drop-in."; removed=true
    fi

    # Undo the SELinux file-context rule this script registered for local-path.
    if command -v semanage >/dev/null && [[ -n "${LOCAL_PATH_DIR:-}" ]]; then
      $SUDO semanage fcontext -d "${LOCAL_PATH_DIR}(/.*)?" >/dev/null 2>&1 || true
    fi
    if [[ "$removed" == "true" ]]; then
      warn "Reboot this host before installing Kubernetes again."
      hint "The uninstallers do not clear every netfilter table and virtual interface"
      hint "they created. A cluster installed over that residue comes up with pods that"
      hint "cannot reach the service network, and the chart's certificate and secret"
      hint "Jobs then fail permanently, so it does not recover on its own."
      hint "    sudo systemctl reboot"
    else
      warn "Neither k3s-uninstall.sh nor rke2-uninstall.sh was found; no local cluster was installed by this script."
    fi
    # Undo the firewalld changes so the host is left as it was found.
    if command -v firewall-cmd >/dev/null && $SUDO systemctl is-active --quiet firewalld 2>/dev/null; then
      log "Reverting the firewalld rules"
      $SUDO firewall-cmd --permanent --zone=trusted --remove-source=10.42.0.0/16 >/dev/null 2>&1 || true
      $SUDO firewall-cmd --permanent --zone=trusted --remove-source=10.43.0.0/16 >/dev/null 2>&1 || true
      for prt in 6443/tcp 443/tcp 80/tcp 9345/tcp 30000-32767/tcp; do
        $SUDO firewall-cmd --permanent --remove-port="$prt" >/dev/null 2>&1 || true
      done
      $SUDO firewall-cmd --reload >/dev/null 2>&1 || true
      ok "firewalld rules reverted."
    fi
...
cmd_version() {
  ...
  ensure_kubectl_path
  command -v rke2    >/dev/null && printf 'rke2       %s\n' "$(rke2 --version 2>/dev/null | head -1)"
...
${C_B}INSTALL BEHAVIOUR${C_OFF}
  ...
  --install-k3s / --no-k3s  Force or forbid installing a local cluster
  --selinux <enforcing|permissive>        Default enforcing; permissive is
                            a runtime-only change and is not recommended
  --firewalld <configure|disable|skip>    Default configure: trust the cluster
                            CIDRs and keep the host firewall running
  --expand-disk / --no-expand-disk        Reclaim unallocated LVM space
  --os-upgrade              Run dnf upgrade first (off by default)
...
parse_args() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --k8s)                K8S_DIST="$2"; shift 2 ;;
      --selinux)            SELINUX_MODE="$2"; shift 2 ;;
      --firewalld)          FIREWALLD_MODE="$2"; shift 2 ;;
